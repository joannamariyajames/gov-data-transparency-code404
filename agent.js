...
data = data.filter(d =>
extractYear(d.Year) === detectedYear
)
...
/* If states were mentioned, filter to them */

if(detectedStates.length){

data = data.filter(d =>
detectedStates.includes(d.State)
)
...
const sectorColumns = Object.keys(data[0])
...
insight =
`${highest} spends the most on ${normalize(detectedSector)}. ${lowest} spends the least.`
